...
export default function AuroraNodeCloud({ category, onClose }: NodeCloudProps) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await fetch(`http://localhost:8000/api/subthreads/${category}`);
                const data = await res.json();
                // CHANGE 1: We do NOT add the category title here anymore.
                // Just the raw subthreads.
                setNodes(data);
                setLoading(false);
            } catch (e) {
    ...
    const renderedNodes = useMemo(() => {
        return nodes.map((node, i) => {
            ...
            // CHANGE 2: Offset index by 1.
            // i=0 becomes index=1. This ensures we start the spiral OUTSIDE the center.
            // The center (index 0) remains mathematically empty.
            const index = i + 1;
            ...
            return (
                <div
                    key={node.name}
                    className={`absolute rounded-full border shadow-sm flex flex-col items-center justify-center transition-transform hover:scale-110 hover:z-20 hover:opacity-100 ${nodeStyle}`}
                    style={{
    ...
    }, [nodes, glassClass, containerBorder]);
    ...
    return (
        <>
            ...
            <motion.div
                ...
                className={`fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[100] w-[340px] h-[400px] rounded-[45px] overflow-hidden shadow-2xl flex items-center justify-center border-[1px] ${glassClass} ${containerBorder}`}
                onWheel={handleWheel}
            >
                <button
                    onClick={onClose}
                    className={`absolute top-4 right-4 z-[110] p-2 rounded-full border transition-all active:scale-95 ${glassClass} ${containerBorder} hover:bg-red-500/20 hover:border-red-500/30 group`}
                >
